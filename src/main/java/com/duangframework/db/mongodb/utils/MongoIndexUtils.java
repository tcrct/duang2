...
//        String type = "text".equals(index.type()) ? "1" : "2d";
        String type = index.type().name().toLowerCase();
        ...
        if (!IndexEnums.TEXT.name().equalsIgnoreCase(type)) {
            keys.put(key, type.startsWith("_") ? type.substring(1) : type);
        }
